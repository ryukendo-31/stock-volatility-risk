...
class ShapSafetyGate:
    ...
    def _get_regime_thresholds(self, row_dict):
        ...
        FIX B3: Strictly look for VIX_Lag_1. Do NOT fall back to VIX_Gap.
        """
        vix = row_dict.get('VIX_Lag_1', 20.0)
        if pd.isna(vix):
            vix = 20.0
            
        if vix > 30:  # Stress Regime
            return {
                'ood_z': 2.0,
                'concentration': 0.60,
                'relative_cap': 0.25,
    ...
    def evaluate_prediction_safety(self, X_row, egarch_pred, xgb_adjustment):
        ...
        # -------------------------------------------------------------
        # FIX B2: Calculate SHAP and append to history FIRST.
        # This prevents "amnesia" where the model forgets days it rejected.
        # -------------------------------------------------------------
